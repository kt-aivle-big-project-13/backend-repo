...
public final class ReportPromptTemplate {
    ...
    public static final String SYSTEM_PROMPT = """
            ...
            ### [출력 문법 (Output Syntax)]
            결과물은 PDF·Word 문서로 변환됩니다. 아래 네 가지 문법만 사용하고, 그 밖의 마크다운 문법은 절대 쓰지 마세요.
            변환기가 해석하지 못하는 기호는 문서에 그대로 인쇄되어 보고서 품질을 떨어뜨립니다.

            9. 섹션 제목: `## 1. 감사 개요` 처럼 `##` 과 공백으로 시작하세요. 하위 제목이 필요하면 `###` 을 쓰세요.
            10. 본문: 기호 없이 문단으로 작성하세요. 한 문단은 빈 줄로 구분하세요.
            11. 목록: `- 항목` 형태로만 작성하세요. 번호 목록과 중첩 목록은 쓰지 마세요.
            12. 표: 파이프(|)로 구분하고 헤더 바로 아래에 구분선을 넣으세요. 모든 행의 칸 수를 동일하게 맞추세요.
                | 지표 | 값 | 상태 | 근거 |
                | --- | --- | --- | --- |
                | FIDELITY | 0.92 | 충족 | 기준 0.85 이상 |
            13. 금지 문법: 굵게(**), 기울임(*, _), 취소선(~~), 인라인 코드(`), 코드블록(```), 인용(>), 수평선(---), 링크([]()), 이미지(![]), HTML 태그.
                강조가 필요하면 기호 대신 문장으로 표현하세요.
