...
@Component
public class FastApiComplianceReportClient
        implements ComplianceReportClient {

    private static final String COMPLIANCE_REPORT_PATH =
            "/internal/v1/reports/compliance";

    private final RestClient restClient;

    public FastApiComplianceReportClient(
    ...
    @Override
    public ComplianceReportResponse generate(
            ComplianceReportRequest request
    ) {
        try {
            ComplianceReportResponse response = restClient.post()
                    .uri(COMPLIANCE_REPORT_PATH)
