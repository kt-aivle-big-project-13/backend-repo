...
class SelfCheckLawMappingEventListenerTest {
    ...
    @InjectMocks
    private SelfCheckLawMappingEventListener listener;

    @Test
    void mapsWhenEventReceived() {
        listener.handle(new SelfCheckAnswersSubmittedEvent(AUDIT_ID));

        verify(auditLawMappingService).mapFromSelfCheckAnswers(AUDIT_ID);
    ...
    void swallowsExceptionSoItNeverPropagatesButMarksAuditFailed() {
        willThrow(new RuntimeException("AI 서버 오류"))
                .given(auditLawMappingService).mapFromSelfCheckAnswers(AUDIT_ID);
