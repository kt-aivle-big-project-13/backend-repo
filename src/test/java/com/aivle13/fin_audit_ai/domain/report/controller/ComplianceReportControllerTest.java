...
@Transactional
class ComplianceReportControllerTest extends IntegrationTestSupport {

    private static final String HTML_KEY = "compliance-reports/1/run/report.html";
    private static final String PDF_KEY = "compliance-reports/1/run/report.pdf";
    private static final String WORD_KEY = "compliance-reports/1/run/report.docx";
    ...
    @MockitoBean
    private ComplianceReportClient complianceReportClient;
    ...
    void setUp() {
        UserEntity user = userRepository.save(UserEntity.create(
                "테스트기관", "홍길동", "compliance-report-test@example.com", "hash", UserRole.AUDITOR
        ));
        userId = user.getId();

        otherUserId = userRepository.save(UserEntity.create(
                "타기관", "임꺽정", "compliance-report-other@example.com", "hash", UserRole.AUDITOR
        )).getId();
        ...
        // 규제준수 판정서는 자가점검 응답과 법령 매핑이 판정 근거라, 없으면 조립 단계에서
        // 거부된다. 다른 리포트와 달리 근거를 미리 심어야 한다.
        selfCheckAnswerRepository.save(SelfCheckAnswerEntity.of(
                audit, SelfCheckItemCode.TR_01, SelfCheckAnswerValue.YES
        ));

        LawArticleEntity article = lawArticleRepository.save(LawArticleEntity.of(
                "AI 기본법", "제34조", "고영향 인공지능 사업자의 의무", LocalDate.of(2026, 1, 1)
        ));

        auditRegulationMappingRepository.save(AuditRegulationMappingEntity.of(
                audit, article, ComplianceStatus.COMPLIANT, "자율점검 기반 자동 매칭"
        ));

        when(complianceReportClient.generate(any(ComplianceReportRequest.class)))
                .thenReturn(new ComplianceReportResponse(
                        auditId, HTML_KEY, PDF_KEY, WORD_KEY,
                        "html", 5, 1, 0, "2026-08-05T10:00:00Z", List.of()
                ));
    }
    ...
    void generate() throws Exception {
        mockMvc.perform(post("/api/v1/audits/{auditId}/reports/compliance", auditId)
    ...
    void getLatestDefaultsToHtml() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/audits/{auditId}/reports/compliance", auditId)
                        .with(authentication(asUser())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.format").value("HTML"))
                .andExpect(jsonPath("$.reportType").value("COMPLIANCE_VERDICT"));
    }
    ...
    void downloadPdf() throws Exception {
        ...
        mockMvc.perform(get(
                        "/api/v1/audits/{auditId}/reports/compliance/{reportId}/download",
                ...
                .andExpect(header().string(
                        ...
                        org.hamcrest.Matchers.containsString(
                                "compliance-report-%d.pdf".formatted(auditId))
                ));
    }
    ...
    void downloadWord() throws Exception {
        ...
        mockMvc.perform(get(
                ...
                .andExpect(header().string(
                        "Content-Disposition",
                        org.hamcrest.Matchers.containsString(
                                "compliance-report-%d.docx".formatted(auditId))
                ));
    }
    ...
    private Long reportIdOf(String format) throws Exception {
        String body = mockMvc.perform(
                        get("/api/v1/audits/{auditId}/reports/compliance", auditId)
                                .param("format", format)
                                .with(authentication(asUser())))
                .andExpect(status().isOk())
                .andReturn()
