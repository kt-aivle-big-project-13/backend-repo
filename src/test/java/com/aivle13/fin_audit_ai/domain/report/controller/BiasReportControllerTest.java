...
@Transactional
class BiasReportControllerTest extends IntegrationTestSupport {

    private static final String HTML_KEY = "bias-reports/1/run/report.html";
    private static final String PDF_KEY = "bias-reports/1/run/report.pdf";
    private static final String WORD_KEY = "bias-reports/1/run/report.docx";
    ...
    @MockitoBean
    private BiasReportClient biasReportClient;
    ...
    void setUp() {
        UserEntity user = userRepository.save(UserEntity.create(
                "테스트기관", "홍길동", "bias-report-test@example.com", "hash", UserRole.AUDITOR
        ));
        userId = user.getId();

        otherUserId = userRepository.save(UserEntity.create(
                "타기관", "임꺽정", "bias-report-other@example.com", "hash", UserRole.AUDITOR
        )).getId();
        ...
        when(biasReportClient.generate(any(BiasReportRequest.class)))
                .thenReturn(new BiasReportResponse(
                        auditId, HTML_KEY, PDF_KEY, WORD_KEY,
                        "html", "2026-08-05T10:00:00Z", List.of()
                ));
    }
    ...
    void generate() throws Exception {
        mockMvc.perform(post("/api/v1/audits/{auditId}/reports/bias", auditId)
    ...
    void getLatestDefaultsToHtml() throws Exception {
        ...
        mockMvc.perform(get("/api/v1/audits/{auditId}/reports/bias", auditId)
                        .with(authentication(asUser())))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.format").value("HTML"))
                .andExpect(jsonPath("$.reportType").value("BIAS_REPORT"));
    }
    ...
    void downloadPdf() throws Exception {
        ...
        mockMvc.perform(get(
                        "/api/v1/audits/{auditId}/reports/bias/{reportId}/download",
                ...
                .andExpect(header().string(
                        ...
                        org.hamcrest.Matchers.containsString(
                                "bias-report-%d.pdf".formatted(auditId))
                ));
    }
    ...
    void downloadWord() throws Exception {
        ...
        mockMvc.perform(get(
                ...
                .andExpect(header().string(
                        "Content-Disposition",
                        org.hamcrest.Matchers.containsString(
                                "bias-report-%d.docx".formatted(auditId))
                ));
    }
    ...
    private Long reportIdOf(String format) throws Exception {
        String body = mockMvc.perform(
                        get("/api/v1/audits/{auditId}/reports/bias", auditId)
                                .param("format", format)
                                .with(authentication(asUser())))
                .andExpect(status().isOk())
                .andReturn()
